    ...
    public:
        ...
        void Load(std::string _name, std::string _filepath, TexDataType _type) {
            ...
            if (_type == TexDataType::INT) {
                mData = ImageInterface::LoadImage(_filepath.c_str(), &mWidth, &mHeight, &mChannels);
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, mWidth, mHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, mData);
            } else {
                mDataf = ImageInterface::LoadImagef(_filepath.c_str(), &mWidth, &mHeight, &mChannels);
...
class Cubemap : public Texture {
    ...
    public:
        ...
        void Load(std::string _name, std::string _filepath) {
            mName = _name;
            for (int i = 0; i < 6; i++) {
                mData[i] = ImageInterface::LoadImage(_filepath.c_str(), &mWidth[i], &mHeight[i], &mChannels[i]);
            }
